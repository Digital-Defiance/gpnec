...
final class DualRouteTests: XCTestCase {
    ...
    func testSymmetricCrashFairControl() throws {
    ...
    func testSandboxAsymmetryStillDemonstrable() throws {
        let ctx = try MetalContext()
        let report = try DualRouteVerification(
        ...
        XCTAssertEqual(report.policy, .sandbox)
        XCTAssertTrue(report.verified, report.note)
        XCTAssertGreaterThan(report.poincarePost.delivered, report.euclideanPost.delivered)
    }
}
