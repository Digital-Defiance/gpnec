...
```bash
brew tap digital-defiance/tap
brew install --HEAD gpnec    # until first GitHub Release + bottle
# later: brew install gpnec
open -a "Subspace Lattice"
```

Formula lives in the tap as `Formula/gpnec.rb`. Once a stable release tarball exists,
uncomment `depends_on formula: "gpnec"` on the Lattice cask so installing the game
pulls the accelerator automatically.
